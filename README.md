...
![Coverage](https://raw.githubusercontent.com/endronk/lunch_recipe/master/coverage_badge.svg?sanitize=true)
...
contains the business rule of "usability based on the use-by date".
Therefore, it is not just a dummy model, but rather contains a function
to check if the ingredient can still be used on the selected lunch date.
`FridgeIngredient` is named as fridge ingredient, not just `Ingredient`,
because this ingredient contains `useBy` field, which are only
applicable for ingredient in fridge, and different from ingredient in
recipe, where it doesn't have such property.
